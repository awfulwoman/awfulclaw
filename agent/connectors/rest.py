...
class RESTConnector(Connector):
    ...
    async def _handle_info(self, request: Request) -> JSONResponse:
        ...
        # Prefer agent-authored summary if available (generated by SummaryHandler)
        if self._state_path is not None and name != "checkin":
            summary = self._state_path / "info" / f"{name}.md"
            if summary.is_file():
                return JSONResponse({"content": summary.read_text(encoding="utf-8")})
        # Fall back to raw profile file
